...
void lbm_comm_sync_ghosts_vertical( lbm_comm_t * mesh, Mesh *mesh_to_process, lbm_comm_type_t comm_type, int target_rank, int y )
{
	...
	//{
		//case COMM_SEND:
			...
			lbm_mesh_cell_t line_to_send;
			
			line_to_send = malloc(sizeof(double) * DIRECTIONS * (mesh_to_process->width - 2));
			
			for ( x = 1 ; x < mesh_to_process->width - 1 ; x++)
				memcpy(&line_to_send[(x-1)*DIRECTIONS], Mesh_get_cell(mesh_to_process, x, y), sizeof(double) * DIRECTIONS);
			MPI_Send( &line_to_send[0], DIRECTIONS * (mesh_to_process->width - 2), MPI_DOUBLE, target_rank, 0, MPI_COMM_WORLD);
		...
		//case COMM_RECV:
			...
			lbm_mesh_cell_t line_to_recv;
			
			line_to_recv = malloc(sizeof(double) * DIRECTIONS * (mesh_to_process->width - 2));
			MPI_Recv( &line_to_recv[0], DIRECTIONS * (mesh_to_process->width - 2), MPI_DOUBLE, target_rank, 0, MPI_COMM_WORLD,&status);
			
			for ( x = 1 ; x < mesh_to_process->width - 1 ; x++)
				memcpy(&Mesh_get_cell(mesh_to_process, x, y)[0], &line_to_recv[(x-1)*DIRECTIONS], sizeof(double) * DIRECTIONS);
				//for ( k = 0 ; k < DIRECTIONS ; k++)
